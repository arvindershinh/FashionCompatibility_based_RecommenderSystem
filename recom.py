...
def get_recommendations(images, caption):
    path = ''
    ...
    print(params['visual_semantic'].keys())

    # **TEST FEATURES LOADING**


    test_features_path = os.path.join(path, 'capture/check/train_features.p')
...
# --> MAIN PROGRAM
    ...
    def process(input_images, input_caption, tokenDic, images_embedding_db, images_path, model_resnet, model_params):
        ...
        if position:
            ...
            if position == 'f':
                ...
                seqF1 = updateOutfit_byCaption(seqF1, input_caption, tokenDic, semantic_embedding_params,
                                               image_VSs_unit)
                seqF2 = updateOutfit_byCaption(seqF2, input_caption, tokenDic, semantic_embedding_params,
                                               image_VSs_unit)
            ...
            if position == 'b':
                seqB1, seqB2 = seqB
                seqB1 = updateOutfit_byCaption(seqB1, input_caption, tokenDic, semantic_embedding_params,
                                               image_VSs_unit)
                seqB2 = updateOutfit_byCaption(seqB2, input_caption, tokenDic, semantic_embedding_params,
                                               image_VSs_unit)
...
# --> OUTFIT PREDICTION
    ...
    outfit, outfit_image_paths = process(images, caption_query, wordDic, embedding_features, outfitImages_path, modelResnet, params)
    new_outfits = []
    import random

    for item in outfit_image_paths:
    ...
    outfit_image_paths += new_outfits
    outfit_image_paths = list(set(outfit_image_paths))
    final_images = []
    for file in outfit_image_paths:
